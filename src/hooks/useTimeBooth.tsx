...
export const useTimeBooth = () => {
  ...
  const connectWebRTC = async () => {
    try {
      ...
      conversationRef.current = await Conversation.startSession({
        agentId: state.persona === 'girlfriend' ? 'T3V3l6ob4NjAgncL6RTX' : 'XWcySB7eGEOQPmqtfR3a',
        onConnect: () => {
  ...
  return {
    ...state,
    setYear: (year: number) => setState(prev => ({ ...prev, year })),
    setLocation: (location: string) => setState(prev => ({ ...prev, location })),
    callGirlfriend,
    hangupPhone,
    speak,
    setPersona,
